...
public class DoubleCacheManager {
     ...
     */
    public static final int DISKLRU = 2;
    /**
     ...
     */
    @Nullable
    public static Bitmap get(String key,int model) {
        Bitmap bitmap = null;
        switch (model) {
            case ALL_ALLOW:
                if (lruCacheManager != null && diskLruCacheManager != null) {
     ...
     * @param key 索引
     */
    public static void remove(String key, int model) {
        switch (model) {
            case ALL_ALLOW:
                if (lruCacheManager != null && diskLruCacheManager != null) {
                    lruCacheManager.removeCache(key);
     ...
     * 设置缓存模式
     *
     * @param modelSet LRU、ONLY_DISK、ALL_ALLOW
     */
